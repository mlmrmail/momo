...
function findById(collection, id) {
  return new Promise((resolve, reject) => {
    collection.findById(id, (error, result) => {
      if (_.isEmpty(id)) {
        reject(makeError('id shouldn\'t be empty.', 500));
      }
...
function updateById(collection, id, body) {
  if (_.isEmpty(id)) throw makeError('Missing id parameter', 500);
  if (_.isEmpty(collection)) throw makeError('Missing Collection', 500);
